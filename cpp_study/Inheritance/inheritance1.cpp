...
private:
		...
		int age;
public:
	Person(const char * name, int age) { // c++의 문자열 리터럴은 const char 이기에 const char*로 넘겨야 오류가 발생하지 않음
...
class Chulsoo :public Person { // Chulsoo class가 Person class를 상속받음
private:
	int numOfBooks;
public:
...
private:
	int numOfFlowers;
public:
	Younghee(const char* name, int age, int numOfFlowers) : Person(name, age) {

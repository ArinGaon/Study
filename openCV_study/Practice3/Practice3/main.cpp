...
// 직선 그리기 예제
void drawLines() {
	Mat img(400, 400, CV_8UC3, Scalar(255, 255, 255)); // 400*400사이즈의 흰색 img
	...
	line(img, Point(250, 90), Point(350, 140), Scalar(255, 0, 0), 1, LINE_AA); // AA : 안티엘리어싱
...
void drawPolys() {
	...
	rectangle(img, Rect(50, 150, 100, 50), Scalar(0, 0, 128), -1); // -1은 도형 내부를 Scalar값의 색으로 채움 + 갈색

	// 원과 타원을 그릴 때에는 LINE_AA형식으로 그리는 것이 가독성에 좋음
	circle(img, Point(300, 120), 30, Scalar(255, 255, 0), -1, LINE_AA);
	circle(img, Point(300, 120), 60, Scalar(255, 0, 0), 3, LINE_AA);

	ellipse(img, Point(120, 300), Size(60, 30), 20, 0, 270, Scalar(255, 255, 0), -1, LINE_AA); // 20은 회전각, 270과 360은 표시할 범위각
...
void drawText2() {
	...
	int fontFace = FONT_HERSHEY_TRIPLEX; // fontFace 변수에 글자체 상수 대입
	double fontScale = 2.0;
	int thickness = 1;

	Size sizeText = getTextSize(text, fontFace, fontScale, thickness, 0); 
	// 0 -> baseLine 텍스트 위치를 기준으로 하는 기준선의 y좌표, 필요없으면 0 지정
	Size sizeImg = img.size();

	Point org((sizeImg.width - sizeText.width) / 2, (sizeImg.height + sizeText.height) / 2);
	// sizeText와 sizeImg 크기정보를 이용하여 문자열을 출력할 좌표를 계산
